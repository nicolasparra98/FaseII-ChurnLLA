--CREATE TABLE IF NOT EXISTS "lla_cco_int_stg"."cwp_mob_stg_dashboardinput_dinamico_RJ_v2" AS
...
SELECT DISTINCT CAST(ACCOUNTNO AS VARCHAR(50)) AS AccountNo
FROM "lla_cco_int_ext"."cwp_mov_ext_derecognition"
WHERE dt='2022-01-01' AND RGU_DRC = 'Y'
)
,DerecognitionBaseMonth AS(
SELECT '2022-02-01' AS DRCMonth, CAST(ACCOUNTNO AS VARCHAR(50)) AS AccountNo
FROM "lla_cco_int_ext"."cwp_mov_ext_derecognition" 
WHERE dt='2022-02-01' AND RGU_DRC = 'Y'
)
,MonthCleaning AS(
SELECT m.*
...
,E_Date,Phone_EOM,Mobile_E_MaxStart,E_Mob_Acc_Name,E_Mobile_ID,E_MobileMRC,E_MobileRGUs,E_AvgMobileMRC,E_MobileTenure,Mobile_MRC_Diff
