...
WHERE telefonia='Pospago' AND "unidad de negocio"='1. B2C'
 AND DATE_TRUNC('MONTH', DATE_PARSE(CAST(Date AS VARCHAR(10)), '%Y%m%d'))=DATE('2022-06-01') or DATE_TRUNC('MONTH', DATE_PARSE(CAST(Date AS VARCHAR(10)), '%Y%m%d'))=DATE('2022-05-01')
)
,lag_dna as (
...
SELECT DISTINCT FixedMonth,RejoinerPopFlag,Fixed_PRMonth,FixedAccount,date('2022-06-01') AS Month
FROM RejoinerPopulation
WHERE RejoinerPopFlag=1
AND Fixed_PRMonth=1
AND FixedMonth<>date('2022-06-01')
GROUP BY 1,2,3,4
)
...
where fixedmonth=date('2022-06-01') 
