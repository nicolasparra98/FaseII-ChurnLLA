...
,interactions_panel as (
    select DATE_TRUNC('month',date(INTERACTION_START_TIME)) as inter_month,ACCOUNT_ID as interactions_account_id, 
    ...
    from "db-stage-prod"."interactions_cwp"
        where --DATE_TRUNC('month',date(INTERACTION_START_TIME)) = (select month_analysis from parameters) and 
         ACCOUNT_ID in (select act_acct_cd from join_account_id) and interaction_id is not null 
...
select month,month_map_mac,total_nodes,nodes_higher_than_6_perc,nodes_higher_than_6_perc*100/total_nodes as kpi_percentage
from( select month,month_map_mac,cast(count(distinct hfc_node) as double) as total_nodes,cast(count(distinct case when percentage_accounts_with_tickets > 6 then hfc_node end) as double) as nodes_higher_than_6_perc
    from group_nodegroup by month,2)
)
