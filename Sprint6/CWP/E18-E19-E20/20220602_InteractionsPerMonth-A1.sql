...
,Interactions_per_month as (
select distinct ACCOUNT_ID, interaction_id, INTERACTION_DATE
FROM INTERACTIONS
) 
,Window_day as(
select Distinct date_trunc('month',interaction_date) as InteractionMonth,account_id,Interaction_date,date_add('DAY',-60, interaction_date) as window_day,Interaction_Id
from Interactions_per_month
)
