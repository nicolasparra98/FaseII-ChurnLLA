...
,MAX_SO_CHURN AS(
SELECT DISTINCT rpad(cast(NOMBRE_CONTRATO as varchar) ,10,'0') AS CONTRATOSO, DATE_TRUNC('month',MAX(FECHA_APERTURA)) as DeinstallationMonth, MAX(FECHA_APERTURA) AS FECHA_CHURN
...
,CHURNERSSO AS(
SELECT DISTINCT rpad(cast(NOMBRE_CONTRATO as varchar) ,10,'0') AS CONTRATOSO, DATE_TRUNC('month',FECHA_APERTURA) as DeinstallationMonth,Fecha_apertura as DeinstallationDate
 ,CASE WHEN submotivo='MOROSIDAD' THEN 'Involuntary'
       WHEN submotivo <> 'MOROSIDAD' THEN 'Voluntary'
  END AS Submotivo
FROM ServiceOrders t INNER JOIN MAX_SO_CHURN m on rpad(cast(t.NOMBRE_CONTRATO as varchar) ,10,'0') = m.contratoso and fecha_apertura = fecha_churn
WHERE TIPO_ORDEN = 'DESINSTALACION' AND (ESTADO <> 'CANCELADA' OR ESTADO <> 'ANULADA') AND FECHA_APERTURA IS NOT NULL
--and cast(nombre_contrato as varchar) NOT LIKE '%E%' --temporal
)
,MaximaFecha as(
select distinct rpad(cast(act_acct_cd as varchar),10,'0') as act_acct_cd, max(fecha_extraccion) as MaxFecha 
...
left join churnersso c on contratoso=rpad(cast(f.act_acct_cd as varchar),10,'0') and date_trunc('month',fecha_extraccion)=DeinstallationMonth
left join MaximaFecha m on rpad(cast(f.act_acct_cd as varchar),10,'0')=rpad(cast(m.act_acct_cd as varchar),10,'0')
)
,MaxFechaJoin as(
select Fecha_extraccion,DeinstallationMonth as DxMonth,rpad(cast(act_acct_cd as varchar),10,'0') as act_acct_cd
...
on Fixed_Month=date_trunc('month',b.DxMonth) and rpad(cast(Fixed_Account as varchar),10,'0')=b.act_acct_cd
)
------------------------------------Rejoiners--------------------------------------------------------------
