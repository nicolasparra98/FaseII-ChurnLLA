...
WHERE PD_MIX_CD<>"0P" AND (SAFE_CAST(FI_OUTST_AGE AS numeric)<=90 OR FI_OUTST_AGE IS NULL)
 AND dt='2022-03-02'
GROUP BY DT,FixedMonth,FixedAccount,CONTACTO,FI_OUTST_AGE,TechFlag
)
,HardBundleFlag AS(
...
,CASE WHEN FmcFlag="1.Soft FMC" OR FmcFlag="2.Near FMC" OR FmcFlag="3. Hard Bundle" THEN FmcFlag
      WHEN (FmcFlag="4.PartialConv" OR FmcFlag="5.Fix/Mobile Only") AND P1="FixedOnly" THEN "4.FixedOnly"
...
where fmcflag="MobileOnly"
--)
/*
SELECT DISTINCT FmcFinalFlag,COUNT(DISTINCT FinalAccount) 
FROM FinalFlagFMC
--WHERE FmcFinalFlag="INVESTIGATE"
GROUP BY FmcFinalFlag
*/
...
/*select distinct mobilemonth,fmcflagmob,count(distinct serviceno)
from MobileConvergency 
group by mobilemonth,fmcflagmob*/
