WITH FMC_Table AS
( select distinct month, B_Final_TechFlag, B_FMCSegment, B_FMCType,E_Final_TechFlag, E_FMCSegment, E_FMCType,b_final_tenure,e_final_tenure,B_FixedTenure,E_FixedTenure,finalchurnflag,fixedchurntype,fixedchurnflag,fixedmainmovement,waterfall_flag,finalaccount,fixedaccount,f_activebom
...
,count(distinct finalaccount) as Total_Accounts, count(distinct fixedaccount) as Fixed_Accounts,count(distinct interactions) as UsersInteractions,count(distinct tickets) as UsersTickets, round(sum(NumberTickets),0) as NumberTickets,count(distinct OutlierRepair) as OutlierRepairs,count(distinct userstruckrolls) as UsersTruckrolls,count(distinct missedvisits) as MissedVisits
from final_fields
...
--*/
/*
select distinct month,waterfall_flag,fixedmainmovement,count(distinct finalaccount)
from Final_Fields
where month=date('2022-02-01') and e_fmcsegment is null and e_final_techflag is null and interactionstier='1'
group by 1,2,3
order by 1,2,3
*/
