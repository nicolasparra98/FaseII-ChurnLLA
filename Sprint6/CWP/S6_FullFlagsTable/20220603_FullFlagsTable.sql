WITH FMC_Table AS
( select distinct month, B_Final_TechFlag, B_FMCSegment, B_FMCType,E_Final_TechFlag, E_FMCSegment, E_FMCType,b_final_tenure,e_final_tenure,B_FixedTenure,E_FixedTenure,finalchurnflag,fixedchurnflag,waterfall_flag,finalaccount,fixedaccount
...
,Tickets_Count as (
select distinct InteractionMonth,account_id,count(interaction_id) as Tickets
...
select distinct date_trunc('month',interaction_date) as month,count(distinct interaction_id) AS Tickets
from Users_Tickets
group by 1
)
------------------Repair Times----------------------------
...
,InteractionsTier
from fmc_table f left join interactions_tier i ON f.finalaccount=i.account_id AND f.month=i.InteractionMonth
)
,TicketsTier_Flag AS(
select distinct f.*
,case when i.account_id is not null then finalaccount else null end as Tickets
,Tickets as NumberTickets
,TicketsTier
...
from TicketsTier_Flag f left join Repair_times r ON f.finalaccount=r.account_id AND f.month=r.month
)
,Final_Fields AS(
select distinct month, B_Final_TechFlag, B_FMCSegment, B_FMCType,E_Final_TechFlag, E_FMCSegment,E_FMCType, b_final_tenure,e_final_tenure,B_FixedTenure,E_FixedTenure,finalchurnflag,fixedchurnflag,waterfall_flag,InteractionsTier,TicketsTier,finalaccount,fixedaccount,interactions,tickets,numbertickets as NumberTickets,OutlierRepair
from RepairTimes_Flag
)
--/*
select distinct month, B_Final_TechFlag, B_FMCSegment, B_FMCType,E_Final_TechFlag, E_FMCSegment, E_FMCType,b_final_tenure,e_final_tenure,B_FixedTenure,E_FixedTenure,InteractionsTier,TicketsTier,finalchurnflag,fixedchurnflag,waterfall_flag
,count(distinct finalaccount) as Total_Accounts, count(distinct fixedaccount) as Fixed_Accounts,count(distinct interactions) as UsersInteractions,count(distinct tickets) as UsersTickets, round(sum(NumberTickets),0) as NumberTickets,count(distinct OutlierRepair) as OutlierRepairs
from final_fields
