...
,ChurnedRGUs_SO AS (
SELECT DISTINCT Month,StartDate,Account_id,dxtype
...
,first_value(mixname_adj) over(partition by FixedAccount order by load_dt desc) as LastRGU
--DATE_TRUNC('MONTH',DATE(load_dt))) AS LastRGU
...
--/*
SELECT DISTINCT fixedmonth,count(distinct V.fixedaccount)
--fixedmonth,V.fixedaccount,f_activeeom,fixedchurnflag,fixedchurntype,dxtype,NumRgusLastRecord,ChurnedRGUs,Vol_Flag --* 
--Month,fixedaccount,dxtype,vol_flag,NumRgusLastRecord,ChurnedRGUs
--Month, DxType, Vol_Flag,count(distinct v.fixedaccount) as Records
FROM churnersdna c RIGHT join VoluntaryFlag v on c.fixedaccount=v.fixedaccount and c.fixedmonth=v.month
WHERE month=date('2022-02-01') AND C.fixedaccount is null
and vol_flag=1
GROUP BY 1--,2,3
--ORDER BY V.fixedaccount
--*/
/*
SELECT DISTINCT Month, DxType, Vol_Flag,count(distinct fixedaccount) as Records
FROM VoluntaryFlag
GROUP BY 1,2,3
ORDER BY 1,2,3
*/
