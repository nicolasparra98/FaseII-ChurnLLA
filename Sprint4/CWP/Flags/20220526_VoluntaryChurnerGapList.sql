...
--Select Count(distinct account) from FirsTCUSTRECORD where date(mes) = date('2022-02-01')
 ,NO_OVERDUE AS(
...
,VoluntaryGap AS(
select distinct v.fixedaccount,v.fixedmonth
--count(v.fixedaccount) --FixedChurnType,FixedChurnSubtype,gap,count(distinct fixedaccount) as records
from FinalChurnFlag_SO v RIGHT JOIN ChurnersDNA c ON v.FixedAccount=c.FixedAccount AND v.FixedMonth=c.FixedMonth
where v.fixedmonth=date('2022-02-01')
and v.f_activebom=1
and (v.Fixedchurntype ='2. Fixed Involuntary Churner' OR v.fixedchurntype IS NULL)
)
--,SO AS(
SELECT DISTINCT v.fixedaccount --,s.*
from VoluntaryGap v left JOIN "db-analytics-prod"."so_hdr_cwp" s ON v.FixedAccount=CAST(s.Account_id AS VARCHAR) AND date(DATE_TRUNC('MONTH',s.order_start_date))=v.FixedMonth
where s.account_id is null
--where order_type = 'DEACTIVATION' AND ACCOUNT_TYPE='R' AND ORDER_STATUS='COMPLETED' and cease_reason_code is not null
--)
