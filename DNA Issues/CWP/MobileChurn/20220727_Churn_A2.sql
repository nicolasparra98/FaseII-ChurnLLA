...
FROM "db-analytics-dev"."tbl_postpaid_cwp"
WHERE "biz_unit_d"='B2C' AND ACCOUNT_STATUS IN ('ACTIVE','GROSS_ADDS','PORT_IN', 'RESTRICTED') AND INV_EXP_DT<>'nan' 
AND billcycle IN ('1','2','7','15','21','28') AND  (FLG_OUTSTD_CURR_MO = '1' OR FLG_OUTSTD_CURR_MO = '0') AND  (FLG_OUTSTD_M1 = '1' OR FLG_OUTSTD_M1 = '0') AND  (FLG_OUTSTD_M2 = '1' OR FLG_OUTSTD_M2 = '0') AND  (FLG_OUTSTD_M3 = '1' OR FLG_OUTSTD_M3 = '0') AND CAST(dt as DATE) >= DATE ('2022-01-01')
))
,MONTHS_OD as (
...
CASE WHEN invoutstanding_d = 0 THEN 0 ELSE (CASE WHEN FLG_OUTSTD_CURR_MO = 1 THEN (CASE WHEN FLG_OUTSTD_M1 = 1 THEN (CASE WHEN FLG_OUTSTD_M2 = 1 THEN (CASE WHEN FLG_OUTSTD_M3 = 1 THEN 4 ELSE 3 END) ELSE 2 END) ELSE 1 END) ELSE 0 END) END as MONTHS_OVERDUE FROM clean_inv_data
))
,outstanding_age as (
SELECT *, DATE_DIFF('day',oldest_v1,dt) fi_outsage_V1,DATE_DIFF('day',oldest_v2,dt) fi_outs_age
from (SELECT *, DATE_ADD('month',-MONTHS_OVERDUE,CURR_M0_BILL_DT) as oldest_v1, DATE_ADD('month',-MONTHS_OVERDUE_ADJST,CURR_M0_BILL_DT) as oldest_v2
FROM MONTHS_OD)
)
,InvoluntaryChurners as (
SELECT date_trunc('month',dt) as invmonth,accountno
from outstanding_age
where fi_outs_age=90
)
,ChurnerTypeFlag AS(
SELECT f.*
