...
WHERE u.dt=LAST_DAY(u.dt,Month)
 AND DATE_DIFF(u.DT,(SAFE_CAST(PARSE_DATETIME('%Y.%m.%d %H:%M:%S',INV_PAYMT_DT) AS DATE)),DAY)<=90
...
--SELECT * FROM MainMovementBase

/* 10. Customer Closing Base
SELECT DISTINCT Month,COUNT( account) AS Records
FROM MainMovementBase
WHERE ActiveEOM=1
GROUP BY Month ORDER BY Month
*/
/* 11. RGUs Closing Base
SELECT DISTINCT Month,E_NumRGUs,COUNT(DISTINCT ACCOUNT) AS Rec,(E_NumRGUs*COUNT(DISTINCT ACCOUNT)) AS RGUS
FROM MainMovementBase 
WHERE ActiveEOM=1
GROUP BY Month,E_NumRGUs
ORDER BY Month,E_NumRGUs
*/
/* 2,25,26. CustomerGrossAdds, Upsell and Downsell
SELECT DISTINCT Month,MainMovement,COUNT(DISTINCT Account) AS Records
FROM MainMovementBase
GROUP BY Month,MainMovement
