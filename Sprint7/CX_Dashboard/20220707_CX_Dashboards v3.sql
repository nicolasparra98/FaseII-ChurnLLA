...
------------------------------------New KPIs--------------------------------------------------------------
,payments as(
select distinct month,opco,market,marketsize,product,biz_unit,'pay' as journey_waypoint,'digital_shift' as facet,'%digital_payments' as kpi_name,round(cast(sum(digital) as double)/cast(sum (pymt_cnt) as double)*100,2) as kpi_meas
...
select distinct Month,Opco,Market,MarketSize,Product,Biz_Unit,journey_waypoint,facet,kpi_name,kpi_meas,year(Month) as ref_year,month(month) as ref_mo
from Join_New_KPIs
