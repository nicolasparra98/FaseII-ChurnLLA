...
from(select *,case when b_active_flag is null then 'Inactive' when b_active_flag='DRC' then 'Inactive' else b_active_flag end as b_active_flag_adj,case when e_active_flag is null then 'Inactive' when e_active_flag='DRC' then 'Inactive' else e_active_flag end as e_active_flag_adj
FROM "lla_cco_int_san"."cwc_fix_stg_weekly_churn_input_mayjun_adj"
)
...
,join_so_fixedbase as (
    ...
    when (b.cst_churn_flag = 1 and ((a.MainChurnFlag <>'Involuntary' and a.MainChurnFlag <> 'Voluntary') or a.MainChurnFlag is null) AND B_NumRGUs > E_Numrgus) or a.mainmovement='3.Downsell' then 'CST Churner'
    ...
    when a.mainmovement = '4.New Customer' or (a.mainmovement = '5.Come Back to Life' and (a.mainchurnflag <> 'Early Rejoiner' or a.mainchurnflag is null) and Fixed_RejoinerMonth=0) then 'Gross Add' --revisar
...
when FinalFixedChurnFlag in('Gross Add','Rejoiner') then sum(e_numrgus)
