...
,Churn_flag as(
select distinct v.month as churnmonth,r.*
,case when date_add('month',1,r.month)=v.month then v.fixedaccount else null end as Vol_Churner
from risk_flag r left join VoluntaryChurners v on r.fixedaccount=v.fixedaccount
)
--/*
select distinct month,Risk_Customer,count(distinct fixedaccount) as Total_Accounts,count(distinct vol_churner) as Churners
from Churn_flag
--where risk_customer=1
