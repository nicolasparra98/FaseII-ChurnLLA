...
WHERE telefonia='Pospago' AND "unidad de negocio"='1. B2C'
 AND DATE_TRUNC('MONTH', DATE_PARSE(CAST(Date AS VARCHAR(10)), '%Y%m%d'))=DATE('2022-07-01') or DATE_TRUNC('MONTH', DATE_PARSE(CAST(Date AS VARCHAR(10)), '%Y%m%d'))=DATE('2022-06-01')
)
,lag_dna as (
...
from (select *,
        ...
        and date(dt) between (DATE('2022-07-01') + interval '1' MONTH - interval '1' DAY - interval '2' MONTH) AND  (DATE('2022-07-01') + interval '1' MONTH - interval '1' DAY)) 
)
...
WHEN DATE_DIFF('DAY', MaxStart, DATE(f.DT)) > 180 and DATE_DIFF('DAY', MaxStart, DATE(f.DT)) <= 360  THEN 'Mid-Tenure'
      WHEN DATE_DIFF('DAY', MaxStart, DATE(f.DT))>360 THEN 'Late-Tenure' END AS B_FixedTenure
...
WHEN DATE_DIFF('DAY', DATE(MaxStart), DATE(f.DT)) > 180 AND  DATE_DIFF('DAY', DATE(MaxStart), DATE(f.DT))<=360 THEN 'Mid-Tenure'
      WHEN DATE_DIFF('DAY', DATE(MaxStart), DATE(f.DT))>360 THEN 'Late-Tenure' END AS E_FixedTenure
...
,CASE WHEN RejoinerMonth>=date('2022-07-01') AND RejoinerMonth<=DATE_ADD('MONTH',1,date('2022-07-01')) THEN 1 ELSE 0 END AS Fixed_PRMonth
...
SELECT DISTINCT FixedMonth,RejoinerPopFlag,Fixed_PRMonth,FixedAccount,date('2022-07-01') AS Month
FROM RejoinerPopulation
WHERE RejoinerPopFlag=1
AND Fixed_PRMonth=1
AND FixedMonth<>date('2022-07-01')
GROUP BY 1,2,3,4
)
...
select distinct fixedmonth,count(distinct fixedaccount),sum(b_numrgus),sum(COALESCE(B_NUMRGUS,0) - coalesce(E_numrgus,0))
--fixedmonth,fixedchurntype,count(distinct fixedaccount),sum(b_numrgus)
FROM FinalChurnFlag_SO
where fixedmonth=date('2022-07-01') 
and f_activebom=1
--and fixedchurntype is not null
and fixedmainmovement='3.Downsell'
group by 1--,2--,3 
order by 1--,2--,3
