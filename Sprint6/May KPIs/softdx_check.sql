WITH FMC_Table AS
( SELECT * FROM
"lla_cco_int_stg"."cwp_sp3_BASEKPIs_dashboardinput_dinamico_RJ_v3_mayo")
...
where month_load between date(first_bill_dt)and date_add('DAY',120, date(sales_month))
...
and month=date('2022-03-01') and F_Salesflag is not null
)
,aa as(
select distinct month,fixedaccount,max(date(load_dt)) as max_load_dt,min(date(act_cust_strt_dt)) as startdate,date_diff('day',min(date(act_cust_strt_dt)),(max(date(load_dt)))) as days
from newsalesjanuary n inner join "db-analytics-prod"."fixed_cwp" d on n.fixedaccount=d.act_acct_cd
where date(load_dt)>=date('2022-05-01')
group by 1,2
)
select distinct month,fixedaccount,max_load_dt,startdate,days
from aa
where days<=46
order by max_load_dt desc
