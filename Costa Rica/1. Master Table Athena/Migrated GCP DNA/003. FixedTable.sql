--create table if not exists "lla_cco_int_stg"."cr_fix_voluntarychurn_duefield_june" as
WITH 
------------------------Fixed Useful Fields -------------------------------------------------------------
UsefulFields AS(
...
WHERE FECHA_EXTRACCION=DATE_TRUNC('month',FECHA_EXTRACCION)  and (mora<=90 or mora is null)
))
,CustomerBase_EOM AS(
select *
...
SELECT DISTINCT reverse(rpad(substr(reverse(nombre_contrato),1,10),10,'0')) as contratoso,DATE_TRUNC('month',FECHA_APERTURA) as DeinstallationMonth,Fecha_apertura as DeinstallationDate,CASE WHEN submotivo='MOROSIDAD' THEN 'Involuntary'
       WHEN submotivo <> 'MOROSIDAD' THEN 'Voluntary' END AS Submotivo,submotivo as submotivo_all
...
--select distinct DeinstallationMonth,submotivo,count(distinct contratoso) from churnersso group by 1,2 order by 1,2
,MaximaFecha as(
...
--select distinct DeinstallationMonth,submotivo,count(distinct act_acct_cd) from churnersjoin group by 1,2 order by 1,2

,MaxFechaJoin as(
select distinct dxmonth as Month,act_acct_Cd as Account
,case when FixedChurnTypeFlag_prel='Voluntary' then '1.Voluntary Churner' else null end as FixedChurnType
--,'1.Voluntary Churner' as FixedChurnType
...
--,submotivo as FixedChurnTypeFlag_prel
,CASE WHEN date_diff('month',DeinstallationMonth,MaxFecha)<=1 THEN Submotivo ELSE NULL END AS FixedChurnTypeFlag_prel
FROM Churnersjoin
WHERE Submotivo IS NOT NULL)
c left join INVOLUNTARYNETCHURNERS i on i.month=c.DxMonth and i.account=c.act_acct_cd
where i.account is null
)
--select distinct month,count(distinct account) from maxfechajoin where FixedChurnType='1.Voluntary Churner'
...
select distinct fixed_month
,fixedchurntype
,count(distinct fixed_account),sum(b_numrgus)
--fixed_month,fixedchurntype,fixed_account,b_numrgus,b_mixcode_adj,b_bundlename
from finaltable
