...
`gcp-bia-tmps-vtr-dev-01.lla_temp_dna_tables.2022-08-04_Cabletica_Final_Sprint7_Table_CX_DashboardInput_v2` AS
...
,ecommerce as(
select distinct month,'CT' as Opco, 'Costa_Rica' as Market,'Large' as MarketSize,'Fixed' as Product,'B2C' as Biz_Unit,'digital_shift' as facet,'buy' as journey_waypoint,'e-Commerce' as kpi_name, null as kpi_meas, null as kpi_num,	null as kpi_den,null as KPI_Sla,'M-0' as Kpi_delay_display,'OVERALL' as Network,extract (year from date(Month)) as ref_year, extract(month from date(month)) as ref_mo from fmc_table)
...
,frccare as(
select distinct month,Opco,Market,MarketSize,Product,Biz_Unit,'effectiveness' as facet,'support-call' as journey_waypoint,kpi_name,kpi_meas,null as kpi_num,null as kpi_den,null as KPI_Sla,Kpi_delay_display,Network,extract (year from date(Month)) as ref_year, extract(month from date(month)) as ref_mo  from wanda_kpis where kpi_name='FRC_Care')
