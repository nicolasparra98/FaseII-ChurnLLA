...
ALTASPLAN2P AS(
    ...
    FROM ALTASINTERNET30  i INNER JOIN ALTASNEXTGENTV t ON i.CONTRATOALTA = t.CONTRATOALTA and i.Formato_Fecha = t.Formato_Fecha
    --WHERE t.CONTRATOALTA IS NULL
    --WHERE i.CONTRATOALTA IS NULL
--Para poner NEXTGEN y MB apagar el 1 when y cambiar left join a inner join 
--Para solo NEXTGEN usar right join  y prender el 2 when 
--Para no NEXTGEN prender el 1 when y poner left join
),
