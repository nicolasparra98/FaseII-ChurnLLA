...
,union_dna as (
    select act_acct_cd, fi_outst_age, date(dt) as dt,pd_mix_cd,pd_bb_accs_media,pd_TV_accs_media,pd_VO_accs_media, act_acct_inst_dt,act_cust_strt_dt,act_cust_typ_nm,date_trunc('month',date(dt)) as Month_load
...
SELECT distinct act_acct_cd, concat(max(act_acct_cd),'-',min(first_oldest_unpaid_bill_dt)) as act_first_bill,date_trunc('month',first_inst_dt) as instmonth
 FROM(
  select act_acct_cd,
    ...
    from 
        (
        select act_acct_cd, fi_outst_age, date(dt) as dt,act_acct_inst_dt,
...
FROM
    ...
    fi_outst_age, date(dt) as dt, pd_mix_cd
    FROM 
        (
        select act_acct_cd, fi_outst_age, date(dt) as dt,pd_mix_cd,pd_bb_accs_media,pd_TV_accs_media,pd_VO_accs_media, act_acct_inst_dt,act_cust_strt_dt,
        case when fi_outst_age is null then '1900-01-01' else cast(date_add('day',-cast(fi_outst_age as int),date(dt)) as varchar) end as oldest_unpaid_bill_dt
        from union_dna
         WHERE act_cust_typ_nm = 'Residencial'
         and act_acct_cd in (select act_acct_cd from monthly_inst_accounts)
...
case when (max_fi_outst_age>=46 and Month_Inst <date('2022-04-01')) or(max_fi_outst_age>=20 and Month_Inst>=date('2022-04-01')) then 1 else 0 end as soft_dx_flg,
