...
-- #################################CHURNERS################################################################
,clean_data as (
SELECT *,CAST (billcycle_temp as BIGINT) as billcycle,CAST(flg_outstd_d_temp as BIGINT) as flg_outstd_d
FROM(SELECT accountno,account_status,biz_unit_d,billcycle as billcycle_temp,flg_outstd_d as flg_outstd_d_temp,CAST(dt as DATE) as dt,CAST (FLG_OUTSTD_CURR_MO as BIGINT) as FLG_OUTSTD_CURR_MO,CAST(date_parse(REPLACE(inv_paymt_dt,'.','-'), '%Y-%m-%d %H:%i:%s') as DATE) as inv_paymt_dt,CAST(date_parse(REPLACE(inv_exp_dt,'.','-'), '%Y-%m-%d %H:%i:%s') as DATE) as inv_exp_dt
FROM "db-analytics-dev"."tbl_postpaid_cwp"
WHERE "biz_unit_d"='B2C' AND ACCOUNT_STATUS IN ('ACTIVE','GROSS_ADDS','PORT_IN', 'RESTRICTED') AND INV_EXP_DT<>'nan' 
AND billcycle IN ('1','2','7','15','21','28') AND  (flg_outstd_d = '1' OR flg_outstd_d = '0')
))
,custom_fields_data as (
SELECT  *, CASE WHEN  CAST(flg_outstd_d as BIGINT) = 1 THEN 1 WHEN  CAST(flg_outstd_d as BIGINT) = 0 AND bill_dt <= dt AND inv_paymt_dt  <= bill_dt THEN 1 ELSE 0 END AS flg_outstd_d_final
FROM (SELECT *,DATE_ADD('day',billcycle-2,DATE_TRUNC('month', DATE_ADD('day',-15,inv_exp_dt))) as bill_dt
FROM clean_data)
)
,fi_outsage as (
...
(SELECT dt,accountno,account_status,biz_unit_d,billcycle,bill_dt,inv_paymt_dt,inv_exp_dt,
   CASE WHEN flg_outstd_d_final = 0 THEN 0 ELSE 
