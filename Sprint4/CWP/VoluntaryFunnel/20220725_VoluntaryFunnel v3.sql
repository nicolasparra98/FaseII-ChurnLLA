...
,Retention as(
select distinct case date(date_parse(cast(month as varchar),'%Y%m%d')) as month,account_number,retenidos,canal,r."razon resumida" as razon_resumida,save_id,case when canal='RCOE' THEN 'RCOE' else 'Other Channels' end as Channel
from "lla_cco_int_san"."cwp_con_ext_reten" r
--where canal='RCOE'
)
,Attempt_Dx_Flag AS(
select distinct f.*
