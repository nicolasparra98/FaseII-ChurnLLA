...
--*/
,involuntary_gap as(
select distinct * --month,fixedmainmovement,fixed_rejoinermonth,rejoinerfmcflag,waterfall_flag,count(distinct act_acct_cd),sum(rejoiners_rgu)
from panel_juan j inner join fmc_table f on j.act_acct_cd=f.fixedaccount
where fixedchurntype='2. Fixed Involuntary Churner' and net_inv_churn_flag=0--and net_inv_churn_flag=0
--where rejoiner_flag = 1 --rejoiners_rgu=-1
--group by 1,2,3,4,5 order by 1,2,3,4,5
)
--,dna_invol_check as(
select distinct date(d.dt),d.act_acct_cd,d.fi_outst_age,d.pd_mix_cd,b_numrgus,net_inv_churn_flag
from "db-analytics-prod"."fixed_cwp" d inner join involuntary_gap i on d.act_acct_cd=i.act_acct_Cd
