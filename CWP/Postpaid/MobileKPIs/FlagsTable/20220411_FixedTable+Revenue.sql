...
FROM SpinMovementBase f LEFT JOIN AllChurners c ON f.FixedAccount = c.ChurnAccount and f.FixedMonth = c.Month
)
...
SELECT --*
FixedMonth,MainMovement,COUNT(DISTINCT FixedAccount) AS Records,ROUND(SUM(E_Fixed_MRC-B_Fixed_MRC),2) AS Revenue
,ROUND(SUM(E_Fixed_MRC-B_Fixed_MRC)/COUNT(DISTINCT FixedAccount),2) AS ARPU
FROM Fixed_Convergency 
--WHERE ChurnFlag="Churner"
GROUP BY FixedMonth,MainMovement
ORDER BY FixedMonth,MainMovement
