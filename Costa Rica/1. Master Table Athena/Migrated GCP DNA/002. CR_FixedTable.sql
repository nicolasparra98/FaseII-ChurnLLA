--create table if not exists "lla_cco_int_stg"."cr_fix_stg_dashboardinput_sprint12_may" as
WITH 
------------------------Fixed Useful Fields -------------------------------------------------------------
UsefulFields AS(
...
WHERE RejoinerPopFlag=1 AND Fixed_PR=1 AND Fixed_Month<>date('2022-05-01')
GROUP BY 1,2,3,4
)
...
Select distinct *
--fixed_month,fixedchurntypeflag,count(distinct fixed_account)
From FinalTable
--group by 1,2
--order by 1,2
--limit 10
