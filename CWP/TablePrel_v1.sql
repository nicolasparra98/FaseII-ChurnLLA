WITH
UsefulFields AS(
SELECT ACT_ACCT_CD, DT, MAX(SAFE_CAST(SAFE_CAST(act_cust_strt_dt AS TIMESTAMP) AS DATE)) AS MaxStart, FI_TOT_MRC_AMT AS mrc_amt
...
GROUP BY ACT_ACCT_CD, DT,numBB,numTV,numVO,mrc_amt
)
,AverageMRC_User AS(
...
,ActiveUsersBOM AS(
SELECT DISTINCT DATE_TRUNC(DATE(DT),Month) AS Month, ACT_ACCT_CD AS accountBOM,dt,(NumBB+NumTV+NumVO) as B_NumRGUs
...
,MaxStart as B_MaxStart, mrc_amt AS B_MRC
FROM UsefulFields
WHERE dt="2022-02-02"
GROUP BY 1,2,3,B_MixName_Adj,B_MixCode_Adj,B_NumRGUs,B_MaxStart,B_MRC
)
,ActiveUsersEOM AS(
SELECT DISTINCT DATE_TRUNC(DATE(DT),MONTH) AS Month, ACT_ACCT_CD AS accountEOM,dt,(NumBB+NumTV+NumVO) as E_NumRGUs
  ,CASE WHEN NumBB = 1 AND NumTV = 0 AND NumVO = 0 THEN "BO"
    WHEN NumBB = 0 AND NumTV = 1 AND NumVO = 0 THEN "TV"
    WHEN NumBB = 0 AND NumTV = 0 AND NumVO = 1 THEN "VO"
    WHEN NumBB = 1 AND NumTV = 1 AND NumVO = 0 THEN "BO+TV"
...
,MaxStart as E_MaxStart, MRC_AMT AS E_MRC
FROM UsefulFields
WHERE dt=LAST_DAY(dt,Month)
GROUP BY 1,2,3,E_MixName_Adj,E_MixCode_Adj,E_NumRGUs,E_MaxStart,E_MRC
)
,CustomerStatus AS(
  SELECT DISTINCT
  ...
  B_MixName_Adj,B_MixCode_Adj,E_MixName_Adj,E_MixCode_Adj,B_NumRGUs,E_NumRGUs,B_MaxStart,E_MaxStart,B_MRC,E_MRC
