--CREATE TABLE IF NOT EXISTS "lla_cco_int_stg"."cwp_fix_stg_dashboardinput_dinamico_RJ_v3_mayo2" AS
WITH
Convergente AS(
...
WHERE telefonia='Pospago' AND "unidad de negocio"='1. B2C'
 AND DATE_TRUNC('MONTH', DATE_PARSE(CAST(Date AS VARCHAR(10)), '%Y%m%d'))=DATE('2022-05-01') or DATE_TRUNC('MONTH', DATE_PARSE(CAST(Date AS VARCHAR(10)), '%Y%m%d'))=DATE('2022-04-01')
)
...
AND date(load_dt) between (DATE('2022-05-01') + interval '1' MONTH - interval '1' DAY - interval '3' MONTH) AND  (DATE('2022-05-01') + interval '1' MONTH - interval '1' DAY + interval '3' MONTH)
...
,FIRSTCUSTRECORD AS (
    ...
    FROM FixedUsefulFields 
    WHERE LOAD_dt = date('2022-04-30')-- date_trunc('MONTH', DATE(LOAD_dt)) + interval '1' MONTH - interval '1' day
    Group by 1,2
)
...
,CASE WHEN RejoinerMonth>=date('2022-05-01') AND RejoinerMonth<=DATE_ADD('MONTH',1,date('2022-05-01')) THEN 1 ELSE 0 END AS Fixed_PRMonth
...
SELECT DISTINCT FixedMonth,RejoinerPopFlag,Fixed_PRMonth,FixedAccount,date('2022-05-01') AS Month
FROM RejoinerPopulation
WHERE RejoinerPopFlag=1
AND Fixed_PRMonth=1
AND FixedMonth<>date('2022-05-01')
GROUP BY 1,2,3,4
)
...
where fixedmonth=date('2022-05-01') and FixedChurnSubtype='Incomplete CST' --and p.FixedChurnSubtype<>'Incomplete CST'
